		...
		public static bool Prefix(Hediff_Pregnant __instance)
		{
            if ((Find.TickManager.TicksAbs % 200 == 0) && (__instance?.pawn?.genes?.HasGene(VRESauridsDefOf.VRESaurids_Oviparous) ?? false))
            {
                try
				{
					...
					// Tell the player.
					LetterMaker.MakeLetter("VRESaurids.EggLaidLabel".Translate(__instance.pawn.LabelShort), "VRESaurids.EggLaidDesc".Translate(__instance.pawn.LabelShort), LetterDefOf.PositiveEvent);
